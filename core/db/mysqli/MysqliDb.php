...
class MysqliDb
{
     ...
     */
    public function __construct($host = null, $username = null, $password = null, $db = null, $port = null,$prefix='', $charset = 'utf8')
    {
        $isSubQuery = false;
    ...
    public function mysqli()
    {
        if (!$this->_mysqli) {
        	/**
        	 * hack by gaorq - mysql连接会丢失，在操作session的时候，待查
        	 */
        	if(defined("LG_MYSQL_MODE")&&LG_MYSQL_MODE=="multi"){
                ...
                $this->host = $masterDBS[$dbNum]['host'];
                $this->username = $masterDBS[$dbNum]['username'];
                $this->password = $masterDBS[$dbNum]['password'];
                $this->db = $masterDBS[$dbNum]['db'];
                $this->port = $masterDBS[$dbNum]['port'];
                $this->charset = $masterDBS[$dbNum]['charset'];
            }else{
                $this->host=LG_MYSQL_HOST;
                $this->username=LG_MYSQL_USER;
                $this->password=LG_MYSQL_PASS;
                $this->db=LG_MYSQL_DBNAME;
                $this->port = LG_MYSQL_PORT;
                $this->charset = LG_MYSQL_CHARSET;
            }
